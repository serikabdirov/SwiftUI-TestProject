...
//  DesignSystem
//
//  Created by Денис Кожухарь on 08.11.2022.
//  Copyright © 2022 Satty Zhuldyz. All rights reserved.
//
...
public extension UIView {
    ...
    func showAutosizingToast(
        ...
        let size = toast.systemLayoutSizeFitting(
            CGSize(width: maxWidth, height: 0),
            withHorizontalFittingPriority: .required,
            verticalFittingPriority: .fittingSizeLevel
        )
        toast.frame.size = CGSize(width: min(maxWidth, size.width), height: size.height)
    ...
    private func showToast(_ toast: UIView, duration: TimeInterval, point: CGPoint) {
        ...
        if ToastManager.shared.isSwipeToDismissEnabled {
            let recognizer = UIPanGestureRecognizer(target: self, action: #selector(UIView.handleToastSwiped(_:)))
            toast.addGestureRecognizer(recognizer)
            toast.isUserInteractionEnabled = true
            toast.isExclusiveTouch = true
        }
    ...
    private func hideToast(_ toast: UIView, fromTap: Bool) {
        ...
        UIView.animate(
            ...
            if let nextToast = self.queue.firstObject as? UIView, let duration = objc_getAssociatedObject(
                nextToast,
                &ToastKeys.duration
            ) as? NSNumber, let point = objc_getAssociatedObject(nextToast, &ToastKeys.point) as? NSValue {
    ...
    private func handleToastSwiped(_ recognizer: UIPanGestureRecognizer) {
            ...
            abs(velocityY) > 300
        {
            UIView.animate(
                ...
                animations: {
                    let maxTranslationY: CGFloat = 100
                    recognizer.view?.transform = CGAffineTransform(
                        translationX: 0,
                        y: max(min(translationY * 5, maxTranslationY), -maxTranslationY)
                    )
